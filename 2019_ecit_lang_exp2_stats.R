...
setwd(path_neat("lang_results/exp2"))
...
for(f_name in file_names){
    ...
    subj_itms_base$nontargref_type = 0
    if (subj_cond == 1) {
        subj_itms_base$nontargref_type[subj_itms_base$block_number %in% c(8, 9)] = 1
    } else {
        subj_itms_base$nontargref_type[subj_itms_base$block_number %in% c(4, 5)] = 1
    }

    # subj_itms_base = subj_itms_base[!subj_itms_base$block_number %in% c(8, 9),] # TODO REMOVE

    # condition 1: real nontarget first; condition 2: pseudo nontarget first
    # "real nontarget" - 0; "pseudo nontarget" - 1

    subj_acc_rates = neatStats::aggr_neat(
        dat = subj_itms_base,
        values = valid_trial,
        method = mean,
        group_by = c("stim_type", "nontargref_type"),
        filt = (rt_start >= 150),
        prefix = "acc_rate"
    )
    ...
    subject_line = table_neat(list(subj_acc_rates, subj_rt_mean, subj_dur_mean, subj_rt_mean_blocks, overall_acc),
                              transpose = TRUE)
    ...
    subject_line$rt_diff_odd_0 = mean(probs_sh_odd$rt_start[probs_sh_odd$nontargref_type == 0]) - mean(irrs_sh_odd$rt_start[irrs_sh_odd$nontargref_type == 0])
    subject_line$rt_diff_even_0 = mean(probs_sh_even$rt_start[probs_sh_even$nontargref_type == 0]) - mean(irrs_sh_even$rt_start[irrs_sh_even$nontargref_type == 0])

    subject_line$rt_diff_odd_1 = mean(probs_sh_odd$rt_start[probs_sh_odd$nontargref_type == 1]) - mean(irrs_sh_odd$rt_start[irrs_sh_odd$nontargref_type == 1])
    subject_line$rt_diff_even_1 = mean(probs_sh_even$rt_start[probs_sh_even$nontargref_type == 1]) - mean(irrs_sh_even$rt_start[irrs_sh_even$nontargref_type == 1])
...
main_cit_merg$rt_B_mean_diffs_9 = main_cit_merg$rt_B_mean_probe_9 - main_cit_merg$rt_B_mean_irrelevant_9
...
full_but_lex = main_cit_data[main_cit_data$correct == 0, ]
...
print(":::::::::::::::::::::::CONDITION - real nontarget (0) vs. pseudo nontarget (1) :::::::::::::::::::::")
...
neatStats::t_neat(full_data$rt_mean_diffs_0, full_data$rt_mean_diffs_1, pair = T, greater = NULL)
...
full_data$rt_mean_diffs_1st = ifelse(full_data$condition == 1, full_data$rt_mean_diffs_0, full_data$rt_mean_diffs_1)
full_data$rt_mean_diffs_2nd = ifelse(full_data$condition == 2, full_data$rt_mean_diffs_0, full_data$rt_mean_diffs_1)

neatStats::t_neat(full_data$rt_mean_diffs_0[full_data$condition == 1], full_data$rt_mean_diffs_1[full_data$condition == 2])

neatStats::t_neat(full_data$rt_mean_diffs_1st[full_data$condition == 1], full_data$rt_mean_diffs_1st[full_data$condition == 2])

neatStats::plot_neat(
    full_data,
    values = c('rt_mean_diffs_0', 'rt_mean_diffs_1'),
    between_vars = 'condition',
    value_names = c(
        rt_mean_diffs_0 = 'real',
        rt_mean_diffs_1 = 'pseudo',
        '1' = 'real first',
        '2' = 'pseudo first'
    ), reverse = T
)
...
neatStats::plot_neat(
    full_data,
    values = c('rt_mean_diffs_1st', 'rt_mean_diffs_2nd'),
    between_vars = 'condition',
    value_names = c(
        rt_mean_diffs_1st = 'first',
        rt_mean_diffs_2nd = 'second',
        '1' = 'real first',
        '2' = 'pseudo first'
    )
)
...
neatStats::plot_neat(
    ...
    values = c(
        ...
        'rt_B_mean_diffs_8',
        'rt_B_mean_diffs_9'
    ), between_vars = 'condition', reverse = T
)
...
neatStats::corr_neat(full_data$rt_mean_diffs_0, full_data$lextale, direction = 'p')
neatStats::corr_neat(full_data$rt_mean_diffs_1, full_data$lextale, direction = 'p')
...
main_results = table_neat(
    list(
        ...
        aggr_neat(full_data, full_data$acc_rate_probe_0 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_irrelevant_0 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_nontargref_0 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_target_0 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_targetref_0 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_probe_1 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_irrelevant_1 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_nontargref_1 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_target_1 * 100, round_to = 1),
        aggr_neat(full_data, full_data$acc_rate_targetref_1 * 100, round_to = 1)
    ),
    to_clipboard = T
)
