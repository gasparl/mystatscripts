...
meta_dat$crowdsourced[grepl( "Geven", meta_dat$study, fixed = T )] = "No"
...
meta_dat$title = gsub('Noordraven & Verschuere',
                      'Noordraven et al.',
                      meta_dat$title)
meta_dat$title = ifelse(meta_dat$crowdsourced == 'Yes', paste0(meta_dat$title,'*'), meta_dat$title)
...
t_neat(meta_dat$m_rt_diff_h1_1,
       meta_dat$m_rt_diff_h2_1, pair = T, nonparametric = T)
t_neat(meta_dat$m_rt_diff_h1_1,
       meta_dat$m_rt_diff_h2_1, pair = T)

t_neat(main_preds$rt_mean_h1_diff[main_preds$cond == 1],
       main_preds$rt_mean_h2_diff[main_preds$cond == 1], pair = T)



auc_h1 = t_neat(main_preds$rt_mean_h1_diff[main_preds$cond == 1],
                main_preds$rt_mean_h1_diff[main_preds$cond == 0], auc_added = T)
auc_full = t_neat(main_preds$rt_mean_diff[main_preds$cond == 1],
                main_preds$rt_mean_diff[main_preds$cond == 0], auc_added = T)
roc_neat(auc_full$roc_obj, auc_h1$roc_obj, pair = T, plot_rocs = T)
