...
dat_prep = function(id,
  ...
  for (subjectid in unique(Data_raw$id)) {
    ...
    if (min(overall_acc$aggr_value) > 0.2) {
      ...
      subj_itms_base$trial = as.numeric(as.character(subj_itms_base$trial))
      halfnum = (max(subj_itms_base$trial) - min(subj_itms_base$trial)) / 2
      subj_rt_mean1 =
        neatStats::aggr_neat(
          dat = subj_itms_base[subj_itms_base$trial < halfnum,],
          values = rt,
          method = mean,
          group_by = c("type"),
          filt = (valid_trial == 1),
    ...
    subject_line$overall_acc_allmain = mean(subj_itms_base$valid_trial[subj_itms_base$type %in% c('probe', 'irrelevant')])
