...
dat_prep = function(id,
  ...
  # subj_itms_base  = Data_raw_test
  if (exists("main_cit_merg")) {
    rm(main_cit_merg)
  }
  for (subjectid in unique(Data_raw$id)) {
    ...
    subject_line = table_neat(list(subj_acc_rates,
                                   subj_rt_mean,
                                   overall_acc),
                              transpose = TRUE)
    basics = subj_itms_base[1, c('id', 'gender', 'age', 'cond', 'multiple_single', 'study')]
    subject_line = cbind(basics, subject_line)
    if (exists("main_cit_merg")) {
...
effectsize_data = function(id,
  ...
  pred_1_d <-
    cohen_d_between(Data_Real$pred_1, Data_Real$cond) #real
  pred_2_d <-
    cohen_d_between(Data_Real$pred_2, Data_Real$cond) #real
  ...
  pred_2_auc = t_neat(
    Data_Real$pred_2[Data_Real$cond == 1],
  ...
  the_versions = c("pred_1",
                   "pred_2")
  
  output <-
    tibble(
      cohens_d = c(-as.numeric(pred_1_d[1]),-as.numeric(pred_2_d[1])),
      variance_d = c(as.numeric(pred_1_d[2]),
