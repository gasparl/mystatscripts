...
setwd(path_neat("lang_results/pilot_exp3"))
file_names = list.files(pattern = "^exp3_DEinHU.*txt$")
...
for(f_name in file_names){
    #f_name = "exp3_DEinHU_yes_L53.75_20200207221729_COW.txt"
    ...
    subj_data$stim_type[subj_data$stim_type == "probe1"] = "theprobe"
    subj_data$stim_type[grepl('^probe', subj_data$stim_type)] = "irrelevant"
    subj_data$stim_type[grepl('^targetflr', subj_data$stim_type)] = "targetflr"
    subj_data$stim_type[grepl('^nontargflr', subj_data$stim_type)] = "nontargflr"
    subj_data$stim_type[subj_data$stim_type == "theprobe"] = "probe"

    subj_itms_base = subj_data[which(
        subj_data$phase == 'main' &
            subj_data$stim_type %in% c("probe", "irrelevant", "targetflr", "nontargflr", "target")
    ), ]
    ...
    subj_itms_base$rt_end[subj_itms_base$rt_end > 1300] = NA
    subj_itms_base$press_duration = subj_itms_base$rt_end - subj_itms_base$rt_start
    ...
    subject_line = table_neat(list(subj_acc_rates, subj_rt_mean, subj_dur_mean, subj_acc_rate_blocks, subj_rt_mean_blocks, subj_dur_mean_blocks, overall_acc), transpose = TRUE)
...
neatStats::t_neat(full_data$rt_mean_probe,
                  full_data$rt_mean_irrelevant,
                  pair = T)
neatStats::t_neat(full_data$rt_mean_probe,
                  full_data$rt_mean_irrelevant,
                  pair = T)

neatStats::t_neat(full_data$acc_rate_probe,
                  full_data$acc_rate_irrelevant,
                  pair = T)
neatStats::t_neat(full_data$acc_rate_probe,
                  full_data$acc_rate_irrelevant,
                  pair = T)

neatStats::t_neat(full_data$dur_mean_probe,
                  full_data$dur_mean_irrelevant,
                  pair = T)
neatStats::t_neat(full_data$dur_mean_probe,
                  full_data$dur_mean_irrelevant,
                  pair = T)
...
neatStats::plot_neat(
    ...
    values = c(
        ...
        'rt_B_mean_diffs_3',
        'rt_B_mean_diffs_4'
    ), between_vars = 'speaker', reverse = T
)
